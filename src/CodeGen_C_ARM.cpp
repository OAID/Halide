...
class CodeGen_C_ARM : public CodeGen_C {
public:

    CodeGen_C_ARM(Target,std::ostream &);
protected:
...
CodeGen_C_ARM::CodeGen_C_ARM(Target t,std::ostream &dest) : CodeGen_C(dest,t,CodeGen_C::OutputKind::CImplementation)
{
   
}
string CodeGen_C_ARM::print_type(Type type, AppendSpaceIfNeeded space_option) {
    if(type.bits() == 32 &&type.lanes()==4)
    {
        return "float32x4_t ";
    }
...
void CodeGen_C_ARM::visit(const Ramp *op) {
}
void CodeGen_C_ARM::visit(const Broadcast *op) {
    Type vector_type = op->type.with_lanes(op->lanes);
    string id_value = print_expr(op->value);
    string rhs;
    if (op->lanes > 1) {
        if(print_type(vector_type)=="float32x4_t " )
        {
...
void CodeGen_C_ARM::visit(const Add * op)
{
    if(op->type.bits() == 32 && op->type.lanes()==4)
    {
        // vector<Expr> matches;
...
void CodeGen_C_ARM::visit(const Load *op) {

    string load_intrinsic="vld1q_f32";
...
void CodeGen_C_ARM::visit(const Store *op) {
    string store_intrinsic="vst1q_f32";
...
std::unique_ptr<CodeGen_C> new_CodeGen_C_ARM(const Target &target, std::ostream & dest) {
    return std::make_unique<CodeGen_C_ARM>(target,dest);
}
#else  // WITH_X86
